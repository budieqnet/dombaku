    <div class="content-wrapper">
                                ...
                                <table id="table" class="table table-bordered table-striped">
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td style="text-align:center"><?=$row['kode_ternak']?></td>
                                            <td style="text-align:center"><?=$row['penyakit']?></td>
                                            <td style="text-align:center"><?=tgl_indo($row['tgl_pengobatan'])?></td>
                                            <td style="text-align:center"><?=$row['dokter_obat']?></td>
                                            <td style="text-align:center">
                                                <button class="btn btn-success btn-sm"><i class="fas fa-pencil-alt" data-toggle="modal" data-target="#update<?=$row['id']?>"></i></button>
                                                <a class="btn btn-danger btn-sm" href="<?=base_url()?>index.php/admin_pengobatankambing/hapus_data/<?=$row['id']?>" onclick="return confirm('Yakin akan menghapus data?');"><i class="far fa-trash-alt"></i></a>
                                            </td>
                                        </tr>
                                    <?php 
                                        $no++;
        ...
        <div class="modal fade" id="tambah">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form method="post" action="<?=base_url()?>index.php/admin_pengobatankambing/proses_tambah" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label for="kode_kambing">Kode Ternak</label>
        ...
        <div class="modal fade" id="update<?=$data['id']?>">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form method="post" action="<?=base_url()?>index.php/admin_pengobatankambing/proses_update" enctype="multipart/form-data">
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <select class="form-control select2bs4" name="kode_kambing" required>
                                    <option value="">--- Pilih Kode ---</option>
                                    <?php foreach ($riwayat_penyakit as $row): ?>
                                    <option value="<?=$row['kode_kambing']?>" <?=($row['kode_kambing']==$data['kode_ternak'])?"selected":""?>><?=$row['kode_kambing']?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="nama_penyakit">Nama Penyakit</label>
                                <input type="text" class="form-control" name="nama_penyakit" value="<?=$data['penyakit']?>" required>
                            </div>
                            <div class="form-group">
                                <label for="tgl_pengobatan">Tanggal Pengobatan</label>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2bs4" name="dokter" required>
                                    <option value="">--- Pilih Dokter ---</option>
                                    <?php foreach ($dokter as $row): ?>
                                    <option value="<?=$row['nama']?>" <?=($row['nama']==$data['dokter_obat'])?"selected":""?>><?=$row['nama']?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="keterangan">Keterangan</label>
                                <textarea class="form-control summernote" name="keterangan"><?=$data['keterangan_obat']?></textarea>
                            </div>
                        </div>
    ...
    $(document).ready(function() 
    {
        ...
        $("#kode_kambing").on('change', function() {
            ...
            $.ajax({
                url:'<?=base_url()?>index.php/admin_penyakitkambing/data_kambing_penyakit',
